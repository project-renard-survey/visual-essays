...
- [ve-vis-network](#ve-vis-network) - Displays a network graph.
...
## ve-vis-network
Creates and associates a network or graph visualization with a text element. Graphs are defined using delimited (comma or tab) text files that include the information for plotting the nodes and edges in the graph. The graph data is loaded from a URL or filepath provided in the `ve-vis-network` tag.

### ve-vis-network attributes
- __file__: The file path or URL to a comma or tab delimited text file containing the graph data.
- __layout__: The layout of network to be displayed. Setting this attribute to `hierarchy` will show data in a hierarchical structure, and setting this attribute to `network` will display data in a non-hierarchical layout. The default is `network`.
- __arrows__: The style of edge to be displayed. Edges can be drawn as simple lines, single directional arrows, or bidirectional arrows. The options for this attribute are `to` (for single directional arrows from source to target), `from`(for single directional arrows from target to source), `to, from` (for bidirectional arrows). If there is no value set for this attribute, it will by default draw lines.

### Example ve-vis-network directives
 - [medici.tsv](https://github.com/JSTOR-Labs/plant-humanities/blob/master/graphs/medici.tsv) - A tab separated text file defining a graph using a mix of ad-hoc nodes and nodes defined using Wikidata entities
 - ```<param ve-vis-network layout="network" file="medici.tsv" arrows="to, from">``` - An example use of the directive in markdown.
